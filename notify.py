...
def get_data_from_git(format_string, commit):
    return subprocess.check_output(['git', 'log', '-1', '--format=format:%s' %
                                    format_string, commit])
...
def post_message(connection, url, success):
    ...
    text = '<%s|Build #%s> %s for project %s on branch %s' % \
           (build_url, build_number, status_text, project, branch)
...
def main():
    ...
    parser.add_argument('--slack-url', help='URL for Slack webhook '
                        '(e.g. /services/ABCDEFGH/IJKLMNOP/12312312312312312321)', required=True)
    parser.add_argument('--message', help='Type of message to send', required=True,
